...
universe = 'ndx100' # ['csi300','csi800', 'ndx100']
...
for seed in [0]:
    model = MASTERModel(
        ...
        n_epochs=n_epoch, lr = lr, GPU = GPU, seed = seed, train_stop_loss_thred = 0.0,
...
print("Testing Score and Label analysis:")
score = predictions['score'].values
label = predictions['label'].values
import numpy as np
import pandas as pd
print("Total elements:", len(score))
print("Score NaNs:", np.isnan(score).sum())
print("Label NaNs:", np.isnan(label).sum())
